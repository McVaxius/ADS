...
public sealed class ObjectivePlannerService
{
        ...
        ObservationSnapshot observation)
    {
        return dungeonFrontierService.CurrentMode switch
        {
            ...
            _ => frontierPoint.IsAreaBoundary
                ? $"No live monsters, follow anchors, or eligible progression interactables are currently visible in {context.CurrentDuty?.EnglishName}, so ADS is seeking the game-authored area boundary marker toward map {frontierPoint.ConnectedMapId} ({frontierPoint.Name}). Proximity does not complete this frontier point; the existing unsafe-transition hold remains authoritative when the crossing begins."
