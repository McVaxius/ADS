        ...
        ObservationSnapshot observation)
    {
        return dungeonFrontierService.CurrentMode switch
        {
            ...
            FrontierMode.TreasureDungeon
                => dungeonFrontierService.TreasureDungeonRole == ADS.Models.TreasureDungeonRole.Follower
                    ? $"No live monsters, follow anchors, or eligible progression interactables are currently visible in {context.CurrentDuty?.EnglishName}, and ADS inferred treasure follower mode from {dungeonFrontierService.TreasureDungeonRoleSource}. ADS is sweeping current-room treasure passage candidate {frontierPoint.Name} ({frontierPoint.TreasurePassageGroup}, room {frontierPoint.TreasureRoomIndex}) instead of treating the fixed opener route as authoritative after the entry/start route. Post-entry follower passage routing requires Y <= 5y, reaches candidates at XZ <= 4y, pushes 10y forward once, and advances room progress monotonically so lower rooms are not retried after catch-up. Retry cycle: {dungeonFrontierService.TreasureFollowerRetryCycle}."
