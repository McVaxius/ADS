...
public sealed class BmrReflectionService : IDisposable
{
    ...
    public bool RequestQueenLunatenderDisabled(bool disabled)
    {
        lock (syncRoot)
        {
            if (configuration.ReflectionQueenLunatenderDisabled == disabled
                && !HasPendingRegistryRestoreLocked()
                && (!disabled || configuration.ReflectionToolsEnabled))
            {
                return true;
            }
    ...
    public bool RequestHuntsDisabled(bool disabled)
    {
        lock (syncRoot)
        {
            if (configuration.ReflectionHuntsDisabled == disabled
                && !HasPendingRegistryRestoreLocked()
                && (!disabled || configuration.ReflectionToolsEnabled))
            {
                return true;
    ...
    public bool RequestMaxLoadDistance(float value)
    {
        ...
        lock (syncRoot)
        {
            var clampedValue = Math.Clamp(value, 0.1f, DefaultFallbackMaxLoadDistance);
            if (configuration.ReflectionToolsEnabled
                && configuration.ReflectionMaxLoadDistanceMinimized
                && Math.Abs(configuration.ReflectionMinimizedMaxLoadDistance - clampedValue) < 0.001f
            ...
            configuration.ReflectionToolsEnabled = true;
            configuration.ReflectionMinimizedMaxLoadDistance = clampedValue;
    ...
    public bool RequestMinimizeMaxLoadDistance()
    {
        lock (syncRoot)
        {
            ...
            if (configuration.ReflectionToolsEnabled
                && configuration.ReflectionMaxLoadDistanceMinimized
                && Math.Abs(configuration.ReflectionMinimizedMaxLoadDistance - minimizedValue) < 0.001f
                && !pendingMaxLoadDistanceRestore)
            {
                return true;
            }
    ...
    public bool RequestResetMaxLoadDistance()
    {
        lock (syncRoot)
        {
            if (!configuration.ReflectionMaxLoadDistanceMinimized
                && !pendingMaxLoadDistanceRestore
                && !configuration.ReflectionHasOriginalMaxLoadDistance)
            {
                return true;
            }
    ...
    private bool HasPendingRegistryRestoreLocked()
        => pendingRegistryRestore || removedRegistryEntries.Count > 0;
