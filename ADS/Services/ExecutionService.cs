    ...
    private void UpdateOwnedPhase(DutyContextSnapshot context, PlannerSnapshot planner, ObservationSnapshot observation, string prefix)
    {
        ...
        if (planner.Mode == PlannerMode.Progression)
        {
            ...
            if (committedInteractable is not null)
            {
                ...
                if (IsStickyInteractableCommitment(committedInteractableObjectiveKind, committedInteractable)
                    && planner.ObjectiveKind == committedInteractableObjectiveKind)
                {
                    var stickyTarget = committedInteractableObjectiveKind == PlannerObjectiveKind.TreasureFollowerLoot
                        ? "treasure follower sack"
                        : "treasure coffer";
                    TryAdvanceInteractableObjective(context, committedInteractable, $"{prefix} Following through on the committed {stickyTarget} so ADS does not bounce between loot and competing objectives.");
                    return;
                }
...
//            && objectiveKind is PlannerObjectiveKind.Monster or PlannerObjectiveKind.BossFightMonster)  //because AI is too stupid to understand
    ...
    private void TryAdvanceFrontierPoint(DutyContextSnapshot context, DungeonFrontierPoint frontierPoint, bool isMapXzDestination, bool isXyzDestination, string prefix)
    {
        ...
        {
            observationMemoryService.MarkTreasureFollowerContainerChecked(
                frontierPoint.MapId == 0 ? context.MapId : frontierPoint.MapId,
                Dalamud.Game.ClientState.Objects.Enums.ObjectKind.Treasure,
                frontierPoint.Name,
                frontierPoint.Position,
                "one-shot follower staging arrival");
    ...
    private void TryAdvanceInteractableObjective(DutyContextSnapshot context, ObservedInteractable observedInteractable, string prefix)
    {
        ...
        if (TryInteractWithObject(gameObject))
        {
            ...
            var isTreasureFollowerLootFollowThrough = isTreasureFollowerLoot
                && IsTreasureFollowerLootSackName(observedInteractable.Name);
            var isTreasureFollowerOneShotContainer = isTreasureFollowerLoot
                && !isTreasureFollowerLootFollowThrough;
            if (observedInteractable.Classification == InteractableClass.TreasureCoffer
                && !isTreasureFollowerLoot)
            {
            ...
            else if (isTreasureFollowerOneShotContainer)
            {
                observationMemoryService.MarkTreasureFollowerContainerChecked(
                    observedInteractable,
                    "one-shot follower direct interact");
            }
            else
            {
                ...
                pendingTreasureFollowerLootFollowThrough = isTreasureFollowerLootFollowThrough;
                pendingProgressionInteractResultUntilUtc = isTreasureFollowerLootFollowThrough
                    ? now + InteractAttemptCooldown
                    : isRequiredLikeInteractable
            ...
            var interactResult = isTreasureFollowerOneShotContainer
                ? $"Direct interact sent to treasure follower container {observedInteractable.Name}; ADS marked it checked until duty reset and will not retry this non-sack container."
                : isTreasureFollowerLootFollowThrough
                    ? $"Direct interact sent to treasure follower sack {observedInteractable.Name}; ADS will keep retrying until it disappears."
                : observedInteractable.Classification switch
                {
            ...
            if (isTreasureFollowerOneShotContainer)
                ClearInteractableCommitment();
    ...
    private static bool IsStickyInteractableCommitment(PlannerObjectiveKind objectiveKind, ObservedInteractable interactable)
    ...
    private static bool IsTreasureFollowerLootSackName(string name)
    ...
    private bool TryHoldTreasureCofferLockedRetry(
        ...
        DateTime now)
    {
        if (committedInteractableObjectiveKind == PlannerObjectiveKind.TreasureFollowerLoot)
            return false;
    ...
    private ObservedInteractable? ResolveCommittedInteractable(DutyContextSnapshot context, ObservationSnapshot observation)
    {
        ...
        {
            if (committedInteractableObjectiveKind == PlannerObjectiveKind.TreasureFollowerLoot
                && observationMemoryService.IsTreasureFollowerContainerChecked(interactable))
            {
                ClearInteractableCommitment();
                return null;
            }
    ...
    private bool TryHoldPendingProgressionInteractResult(DutyContextSnapshot context, PlannerSnapshot planner, ObservationSnapshot observation, string prefix)
    {
        ...
        var pendingObjectiveKind = isTreasureFollowerLootFollowThrough
            ? PlannerObjectiveKind.TreasureFollowerLoot
            ...
            var holdReason = pendingProgressionInteractAfterWaitUntilUtc > now
                ? $"Holding configured post-interact wait on {pendingInteractable.Name} for another {(pendingProgressionInteractAfterWaitUntilUtc - now).TotalSeconds:0.0}s before ADS retries or replans."
                    : isRequiredFollowThrough
                        ...
                        : isTreasureFollowerLootFollowThrough
                            ? $"Waiting for treasure follower sack result on {pendingInteractable.Name}; ADS will retry on cooldown until it disappears."
        ...
        if (isTreasureFollowerLootFollowThrough && pendingLiveInteractable is not null)
        {
            TryAdvanceInteractableObjective(
                context,
                pendingLiveInteractable,
                $"{prefix} Treasure follower sack follow-through is still live and targetable, so ADS is retrying it on cooldown.");
            return true;
        }
        ...
        if (!isTreasureFollowerLootFollowThrough)
            observationMemoryService.MarkProgressionInteractionSent(context, pendingInteractable);
        TryRetirePendingSatisfiedManualDestination(pendingInteractable, objectTable.LocalPlayer?.Position);
        ClearInteractableCommitment();
        StopMovementAssists();
        var completionReason = isRequiredFollowThrough
            ...
            : isTreasureFollowerLootFollowThrough
                ? $"Treasure follower sack follow-through finished for {pendingInteractable.Name}; the object is gone, so ADS is waiting for refreshed duty truth before selecting another objective."
                : isExpendableFollowThrough
    ...
    private ObservedInteractable? ResolveObservedInteractable(PlannerSnapshot planner, ObservationSnapshot observation)
    {
        var expectedClass = planner.ObjectiveKind switch
        {
            ...
            PlannerObjectiveKind.TreasureFollowerLoot => InteractableClass.TreasureCoffer,
        ...
        if (planner.ObjectiveKind == PlannerObjectiveKind.TreasureFollowerLoot)
            candidates = candidates.Where(x => !observationMemoryService.IsTreasureFollowerContainerChecked(x));
