...
public sealed class ObservationMemoryService
{
    ...
    private readonly HashSet<string> checkedTreasureFollowerContainers = new(StringComparer.OrdinalIgnoreCase);
        ...
        if (!context.PluginEnabled || !context.IsLoggedIn || !context.InInstancedDuty)
        {
            if (!loggedReset
                && (knownMonsters.Count > 0
                    ...
                    || checkedTreasureFollowerContainers.Count > 0))
            {
    ...
    public void Reset()
    {
        ...
        checkedTreasureFollowerContainers.Clear();
    ...
    public bool IsTreasureFollowerContainerChecked(ObservedInteractable interactable)
    {
        if (interactable.Classification != InteractableClass.TreasureCoffer
            || LooksLikeTreasureSackName(interactable.Name))
        {
            return false;
        }

        var key = BuildTreasureFollowerContainerCheckKey(
            interactable.MapId,
            interactable.ObjectKind,
            interactable.Name,
            interactable.Position);
        return checkedTreasureFollowerContainers.Contains(key);
    }

    public void MarkTreasureFollowerContainerChecked(ObservedInteractable interactable, string reason)
    {
        if (interactable.Classification != InteractableClass.TreasureCoffer
            || LooksLikeTreasureSackName(interactable.Name))
            return;

        MarkTreasureFollowerContainerChecked(
            interactable.MapId,
            interactable.ObjectKind,
            interactable.Name,
            interactable.Position,
        ...
        knownInteractables[interactable.Key] = CreateConsumedInteractable(interactable);
    }

    public void MarkTreasureFollowerContainerChecked(uint mapId, ObjectKind objectKind, string name, Vector3 position, string reason)
    {
        var containerName = NormalizeTreasureFollowerContainerName(name);
        if (string.IsNullOrWhiteSpace(containerName)
            || LooksLikeTreasureSackName(containerName))
        {
            return;
        }

        var key = BuildTreasureFollowerContainerCheckKey(mapId, objectKind, containerName, position);
        if (checkedTreasureFollowerContainers.Add(key))
        {
            log.Information($"[ADS] Marked treasure follower container {containerName} at {Quantize(position)} checked until duty reset after {reason}.");
        }
    }
    ...
    private static string BuildTreasureFollowerContainerCheckKey(uint mapId, ObjectKind objectKind, string name, Vector3 position)
        => $"{mapId}:{objectKind}:{NormalizeTreasureFollowerContainerName(name)}:{Quantize(position)}";

    private static string NormalizeTreasureFollowerContainerName(string name)
    {
        var normalized = string.Join(' ', name.Split(' ', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries));
        const string stagingPrefix = " staging coffer: ";
        var stagingIndex = normalized.IndexOf(stagingPrefix, StringComparison.OrdinalIgnoreCase);
        if (stagingIndex >= 0)
            normalized = normalized[(stagingIndex + stagingPrefix.Length)..].Trim();
    ...
    private InteractableClass ClassifyInteractable(IGameObject gameObject, string name, DutyContextSnapshot context, bool considerTreasureCoffers)
    {
        ...
        if (considerTreasureCoffers && LooksLikeTreasureLootName(context, name))
    ...
    private static bool LooksLikeTreasureLootName(DutyContextSnapshot context, string name)
