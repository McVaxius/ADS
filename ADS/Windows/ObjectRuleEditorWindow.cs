...
public sealed class ObjectRuleEditorWindow : PositionedWindow, IDisposable
{
    ...
    public override void Draw()
    {
        ...
        ImGui.TextWrapped("Coords is now the single coordinate field. Enter `a,b` for map X,Z and `a,b,c` for world X,Y,Z. On manual destination rows, Coords drives MapXzDestination / MapXzForceMarch versus XYZ / XYZForceMarch. ForceMarch rows are authored inside-duty bypass waypoints that can stay committed through incidental combat; they are not Praetorium-mounted-only rows. On ordinary rows, Coords is the positional selector and R is its optional radius.");
    ...
    private IReadOnlyList<int> BuildVisibleRuleIndices()
    {
        ...
        indices = indices.Where(index => MatchesScopeFilter(draft.Rules[index], context, filterMode));
