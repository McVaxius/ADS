...
public sealed class LootWindow : PositionedWindow, IDisposable
{
    ...
    {
        var registrable = plugin.Configuration.LootRegistrableNeedingEnabled;
        if (ImGui.Checkbox("Registrable Need override", ref registrable))
            plugin.SetLootRegistrableNeedingEnabled(registrable);
