...
- Dialog rules now auto-reload while ADS is running; these are global prompt matches that only fire while ADS owns a supported duty. `SelectYesno` remains the default addon, with optional delay and minimized-notification restore callbacks.
